...
app.get('/api/weather/coords', async (req, res) => {
  try {
    // Read latitude and longitude from the query string (e.g., /api/weather/coords?lat=...&lon=...)
    ...
    // The rest of the logic is identical to the city route, just with different URL params
...
app.get('/api/weather/:city', async (req, res) => {
  try {
    ...
    // This is the successful fulfillment of the request.
    // We send a 200 (OK) status code by default, along with the
    // neatly packaged weather data in a JSON object.
    res.json({ currentWeather, forecast });
